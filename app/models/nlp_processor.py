...
class NLPProcessor:
    def __init__(self, model_name="microsoft/phi-3-mini-4k-instruct", use_gpu=True, load_in_4bit=True):
        """
    ...
    def process_query(self, query):
        ...
        # Extract the constraints dictionary using regex
        try:
            ...
            if dict_match:
                ...
                # Prepare constraints for MEGNet+VAE model
                return self._prepare_for_megnet_vae(constraints)
            else:
                logger.error(f"Failed to extract constraints from response")
                return {}
        except Exception as e:
            logger.error(f"Error parsing NLP output: {str(e)}")
            return {}
    ...
    def _prepare_for_megnet_vae(self, constraints):
        ...
        # Filter to only include the three properties our model knows about
        megnet_constraints = {}
        
        for prop, default_range in default_constraints.items():
            if prop in constraints:
                megnet_constraints[prop] = constraints[prop]
            else:
                megnet_constraints[prop] = default_range
